...
to sign and the adjugate prefactor), so $q_i$ has degree
$\le d - 1$, and $p_i := X \cdot q_i$ has degree
$\le d$. Furthermore:
\[
...
$\tau_{m+1, \cdot, l}$ entries with rational coefficients arising
from the basis structure constants $\xi_{j,l}$ of
\cref{lem:henselianPair-r-n-minus-r-1-in-gamma-finsupp} ---
i.e.\ a concrete, computable formula.
...
$\mathrm{adj}(M)_{il}\,\xi_{m,j,l}$ structure constants (no
ideal-power loss). The level cap $m \le d - 1$ is the
...
mechanically by the C--H collapse: $q_i$ has degree $\le d - 1$
in $X$, so its $A$-coefficients are determined by the
$\alpha_i^0, \alpha_i^1, \ldots, \alpha_i^{d-1}$ slots, and each
...
$\mathfrak m_A^{d+1}$ by Step Q5.b, and the resulting equality
in $A$ is
\[
\alpha_i \cdot q_i.\mathrm{eval}\,\alpha_i \;=\; 0 \quad\text{in } A.
\]
...
The four polynomial-bundle properties consumed by the helper
\cref{lem:henselianPair-alpha-zero-via-per-coord-henselian} are
then satisfied as follows:
\begin{enumerate}
\item $p_i.\mathrm{Monic}$ — by construction of $q_i$ at Step Q7
the leading coefficient of $p_i = X \cdot q_i$ at degree $d + 1$
is the $X^d$-coefficient of $q_i$, which is $-\kappa_{i, d-1}$
(or its sign-adjusted form); normalise by scaling
$q_i \mapsto q_i / (\text{leading unit})$ if needed (the leading
coefficient is a unit by inheritance from $\det M$ being a unit;
scaling preserves Steps P1--P5).
...
\uses{lem:henselianPair-cayley-hamilton-power-expansion,
      lem:henselianPair-exists-mAB-pow-decomposition-in-basis,
      lem:henselianPair-mult-det-isUnit-of-isUnit-mod-maximal,
      lem:henselianPair-r-n-minus-r-1-in-gamma-finsupp,
      def:henselianPair-kappa-coefficient}
...
\begin{itemize}
\item ($\mathrm{Monic}$) $p_i$ is monic;
...
\textbf{Carrier accounting (iter-099).} The discharge above uses
only the four polynomial-bundle carriers
(\textsc{Monic}, $\mathrm{eval}\text{-}\alpha\text{-}\mathrm{zero}$,
