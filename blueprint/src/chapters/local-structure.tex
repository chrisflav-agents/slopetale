...
\begin{definition} [Stone-Čech compactification]
  ...
  compact Hausdorff space \(\beta(X)\) such that
  \begin{enumerate}
...
\begin{theorem}
    Let \(X\) be a discrete topological space. Then the Stone-Čech compactification \(\beta(X)\) is extremally disconnected.
    \label{thm:stone-cech-extremally-disconnected}
    \uses{def:extremally-disconnected, def:stone-cech-compactification, thm:extremally-disconnected-projective}
    \lean{StoneCech.projective,CompactT2.Projective.extremallyDisconnected}
    \mathlibok
    \leanok
\end{theorem}
...
\begin{proposition}[{\cite[\href{https://stacks.math.columbia.edu/tag/090D}{Tag 090D}]{stacks-project}}]
  ...
  \lean{CompHaus.projectivePresentation,CompHaus.projective_ultrafilter}
  \mathlibok
  \leanok
\end{proposition}
...
\begin{proof}
  \mathlibok
  \leanok
  \uses{def:extremally-disconnected,thm:extremally-disconnected-projective}
  Let $X'$ be the space of ultrafilters $\operatorname{Ultrafilter}(X)$ on the underlying set of $X$; equivalently, this is the Stone-Čech compactification $\beta(Y)$ of the set $Y = X$ endowed with the discrete topology. It is a projective object in the category of quasi-compact Hausdorff spaces, hence extremally disconnected by \Cref{thm:extremally-disconnected-projective}. The ultrafilter extension of the identity map yields a continuous surjection $X' \to X$.
\end{proof}
