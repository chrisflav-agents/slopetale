...
[\stacksproject{092P}]
    ...
    Let $L/K$ be an extension of fields. If the multiplication map $L \otimes_K L \to L$ is
    flat, then $L$ is a separable algebraic extension of $K$.
\end{lemma}

% NOTE: iter-021 rewrite — the proof prose below now follows the Stacks
% 092K subalgebra-descent route, matching the formalisation strategy. The
% prior polyEquivTensor / "K[X] ⊗_K L ≃ L[X] domain" sketch produced five
% documented dead-ends across iter-017–020 and was abandoned. The argument
% factors through \Cref{lem:weakly-etale-adjoin-singleton} (= the file
% `Proetale/Mathlib/RingTheory/WeaklyEtale/Subalgebra.lean`, born iter-021).
\begin{proof}

\leanok
    \uses{lem:weakly-etale-adjoin-singleton,
          lem:weakly-etale-preserves-absolutely-flat,
          lem:reduced-prime-maximal-of-absolutely-flat}
    By hypothesis $\mu_L : L \otimes_K L \to L$ is flat, so $K \to L$ is weakly étale.
    We prove every $a \in L$ is algebraic over $K$; the separability conclusion then follows
    from the standard fact that an element with a separable minimal polynomial over a perfect
    base is separable, together with the case analysis below.

    Fix $a \in L$ and let $K[a] = \mathrm{adjoin}_K\{a\} \subseteq L$ denote the $K$-subalgebra
    generated by $a$. By \Cref{lem:weakly-etale-adjoin-singleton}, $K \to K[a]$ is weakly étale.
    In particular the multiplication map $\mu_{K[a]} : K[a] \otimes_K K[a] \to K[a]$ is a
    surjective $K$-algebra homomorphism whose kernel is generated by a single idempotent
    (apply \texttt{Algebra.FormallyEtale.iff\_of\_surjective}).

    Now suppose for contradiction that $a$ is transcendental over $K$. Then
    $K[a] \cong K[X]$ and $K[a] \otimes_K K[a] \cong K[X, Y]$. The ambient ring
    $K[X, Y]$ is a domain, so its only idempotents are $0$ and $1$. On the other
    hand $\ker \mu_{K[a]}$ corresponds under the isomorphism to the principal ideal
    $(X - Y) \subseteq K[X, Y]$, which is neither $(0)$ (since $X - Y \neq 0$) nor
    $(1)$ (since $X - Y$ is not a unit). This contradicts the idempotency conclusion
    of the previous paragraph.

    Therefore $a$ is algebraic over $K$. Since $a \in L$ was arbitrary, $L/K$ is algebraic.

    For separability, observe that in the algebraic case the residue extension
    $K \hookrightarrow K[a] = K(a)$ has $K(a) \otimes_K K(a)$ reduced (it is a quotient
    of $L \otimes_K L$, which is reduced as the diagonal idempotent splits the kernel of
    $\mu_L$ and the quotient is a field). Hence $K(a)/K$ is separable
    (Stacks tag \stacksproject{030K}). $L$ is the union of its finitely generated subextensions
    $K(a)$, so $L/K$ is separable algebraic.

    Full details: \cite[\href{https://stacks.math.columbia.edu/tag/092P}{Tag 092P}]{stacks-project},
    using the subalgebra descent of
    \cite[\href{https://stacks.math.columbia.edu/tag/092K}{Tag 092K}]{stacks-project}.
\end{proof}

\begin{theorem}\leanok
[\stacksproject{092Q}, (2) => (3) and last part]
    \label{thm:weakly-etale-imples-ind-etale-over-fields}
    \lean{Algebra.WeaklyEtale.indEtale_of_isField}
...
\leanok
    ...
    \lean{Algebra.WeaklyEtale.exists_indEtale}
\end{theorem}

\begin{proof}
    \leanok
    \uses{thm:weakly-etale-over-sh, prop:bijective-on-stalks-ind-zariski,
    lem:weakly-etale-comp, thm:ind-etale-w-contractible-cover-exists,
    lem:ind-etale-of-weakly-etale-w-contractible}
    Pick a faithfully flat ind-étale $S$-algebra $T$ with $T$ w-contractible (Stacks 0983,
    \Cref{thm:ind-etale-w-contractible-cover-exists}). Equip $T$ with its
    induced $R$-algebra structure via $R \to S \to T$. Since $S \to T$ is ind-étale, hence
    weakly étale, and $R \to S$ is weakly étale by assumption, the composition $R \to T$
    is weakly étale (\Cref{lem:weakly-etale-comp}). It remains to show $R \to T$ is
    ind-étale; this is the content of the auxiliary lemma
    \Cref{lem:ind-etale-of-weakly-etale-w-contractible} below (proved via the local
    structure at each maximal of $T$, using
    \Cref{thm:weakly-etale-over-sh} that weakly-étale over a strictly Henselian local
    ring is an isomorphism).
\end{proof}

\begin{lemma}
    \label{lem:ind-etale-of-weakly-etale-w-contractible}
    \lean{Algebra.IndEtale.of_weaklyEtale_isWContractibleRing}
    \leanok
    Let $R \to T$ be weakly étale with $T$ w-contractible. Then $R \to T$ is ind-étale.
\end{lemma}

\begin{proof}
    \uses{thm:weakly-etale-over-sh, prop:bijective-on-stalks-ind-zariski}
    \leanok
    For each maximal ideal $\mathfrak{m} \subset T$ the local ring $T_\mathfrak{m}$ is
    strictly Henselian (a w-contractible ring is w-strictly-local;
    \Cref{thm:weakly-etale-over-sh} then forces $R_{\mathfrak{p}_\mathfrak{m}} \to
    T_\mathfrak{m}$ to be an isomorphism for $\mathfrak{p}_\mathfrak{m} = \mathfrak{m}
    \cap R$). Combining the local pieces via the bijective-on-stalks / ind-Zariski
    structure of $T$ (\Cref{prop:bijective-on-stalks-ind-zariski}) yields a factoring of
    $R \to T$ through an ind-étale subalgebra, i.e.\ $R \to T$ is ind-étale.
\end{proof}
