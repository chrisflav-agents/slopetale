...
% archon:covers Proetale/Algebra/WeaklyEtaleField.lean Proetale/Mathlib/RingTheory/WeaklyEtale/GoDown.lean
...
\begin{proof}
    ...
    \begin{lemma}[Purity-descent of pure ideals along flat extension of commutative rings]
      ...
      % NOTE iter-153 (blueprint-doctor cleanup): the cross-reference
      % \uses{def:pure-ideal} was removed because no \label{def:pure-ideal}
      % exists in the blueprint tree (no project-side definition of pure
      % ideal; the lemma is documentary-only per the iter-147 mathlib-
      % analogist verdict above and the body explains the meaning inline).
    \end{lemma}
    \begin{proof}
    ...
    \cref{lem:more-on-local-structure-pure-ideal-descent-along-flat}
    as currently stated. The corrected, non-circular cycle-breaker is
    described in the next subsection
    (\emph{Corrected path forward (iter-148): Stacks
    \texttt{lemma-go-down}}), which supersedes the broken descent as
    the active route for the L195 sub-sorry.

    % SOURCE: \stacksproject{04PV} = Lemma 15.106.13 ("\texttt{lemma-go-down}")
    %         in chapter "More on Algebra" §15.106 "Weakly étale ring maps"
    %         (read from references/stacks-09xi-more-algebra-full.tex,
    %         lines 30373--30401)
    % SOURCE QUOTE: Let $A \to B \to C$ be ring maps.
    %   \begin{enumerate}
    ...
    %   \end{enumerate}
    \medskip
    \noindent \emph{Corrected path forward (iter-148): Stacks
    \texttt{lemma-go-down}.} The iter-148 plan agent confirmed Stacks
    tag \stacksproject{04PV} (\texttt{lemma-go-down} in \emph{More on
    Algebra} §15.106) as the non-circular cycle-breaker for the L195
    sub-sorry. The Stacks statement, reproduced verbatim from
    \texttt{references/stacks-09xi-more-algebra-full.tex} lines
    30373--30401:

    \begin{quote}
    \textit{Source: Stacks Project, Tag 04PV.}
    Let $A \to B \to C$ be ring maps.
    \begin{enumerate}
        \item If $B \to C$ is faithfully flat and
        $C \otimes_A C \to C$ is flat, then $B \otimes_A B \to B$
        is flat.
        \item If $B \to C$ is faithfully flat and $A \to C$ is weakly
        étale, then $A \to B$ is weakly étale.
    \end{enumerate}
    \end{quote}

    The Stacks proof of part (1) deduces flatness of $B$ as a
    $B \otimes_A B$-module from flatness of $C$ as a $B \otimes_A B$-module
    via the faithfully flat descent of flatness (Stacks
    \texttt{algebra-lemma-flatness-descends-more-general}); part (2)
    combines (1) with the standard fact that $A \to B$ is flat if
    $A \to C$ is flat and $B \to C$ is faithfully flat. Neither part
    invokes \texttt{etale\_of\_fg},
    \Cref{lem:flat-tensor-over-field-imples-algebraic}, or
    \texttt{isAlgebraic\_of\_weaklyEtale}, so the lemma sits cleanly
    outside the WeaklyEtale forward-reference cycle.

    \medskip
    \emph{Application to the L195 sub-sorry.} Fix $a \in R$ and the
    prime $\mathfrak q \subset R$. Let
    $L := \kappa(\mathfrak q) = \texttt{Localization.AtPrime } \mathfrak q$
    (a field, since $R$ is absolutely flat, by
    \Cref{lem:reduced-prime-maximal-of-absolutely-flat} and
    \Cref{lemma:reduced-prime-maximal-field}). Let
    $a' := \mathrm{algebraMap}\,R\,L\,a \in L$, and let
    $L' := K(a') \subseteq L$ be the finitely generated $K$-subfield
    of $L$ generated by $a'$.

    The chain $K \to L' \to L$ satisfies the hypotheses of Stacks
    \stacksproject{04PV} part (2):
    \begin{enumerate}
        \item $L' \to L$ is faithfully flat, since every nonzero field
        extension is faithfully flat (Mathlib's
        \texttt{Module.FaithfullyFlat} instance for field extensions).
        \item $K \to L$ is weakly étale: the framing paragraph above
        showed that $K \to R$ is weakly étale (hypothesis) and
        $R \to L$ is weakly étale (the localization at a prime is
        weakly étale, via
        \Cref{lemma:etale-weakly-etale-algebra}); composing with
        \Cref{lem:weakly-etale-comp} gives $K \to L$ weakly étale.
    \end{enumerate}
    Hence $K \to L'$ is weakly étale.

    \medskip
    \emph{Step 2 (formally unramified).} Since $K \to L'$ is weakly
    étale, the multiplication map
    $L' \otimes_K L' \twoheadrightarrow L'$ is a flat surjection;
    its kernel $I_{L'}$ is therefore a pure ideal of
    $L' \otimes_K L'$, and pure ideals in commutative rings are
    idempotent (Stacks \texttt{algebra-lemma-pure}). The standard
    identification $\Omega_{L'/K} = I_{L'} / I_{L'}^2$
    then yields $\Omega_{L'/K} = 0$, i.e. $K \to L'$ is formally
    unramified.

    \medskip
    \emph{Step 3 (fg formally unramified field $\Rightarrow$ finite
    separable).} Since $a'$ is a single element, $L' = K(a')$ is
    finitely generated as a $K$-algebra (it is either
    $K[X]/\min_K(a')$ if $a'$ is algebraic, or the field of fractions
    of $K[a']$ — a localization of $K[a']$ — if $a'$ is
    transcendental). In either case $L'$ is essentially of finite
    type over $K$, so Mathlib's
    \texttt{Algebra.FormallyUnramified.isSeparable} applies and
    yields \texttt{Algebra.IsSeparable K L'}. The classical
    consequence is that $L'/K$ is finite separable; in particular
    $a' \in L'$ is algebraic over $K$.

    Transferring algebraicity along the injection
    $L' \hookrightarrow L$ via \texttt{IsAlgebraic.of\_injective}
    (Mathlib's transfer of algebraicity along ring-map injections)
    yields \texttt{IsAlgebraic K aLoc} where
    $\mathrm{aLoc} = \mathrm{algebraMap}\,R\,L\,a$, closing the
    L195 sub-sorry.

    \medskip
    \emph{Lean target staging.} The new Lean carrier required is
    Stacks \stacksproject{04PV} part (2), packaged as:

    \begin{quote}
    \texttt{lemma Algebra.WeaklyEtale.of\_faithfullyFlat\_intermediate}\\
    \texttt{\{K B C : Type*\} [CommRing K] [CommRing B] [CommRing C]}\\
    \texttt{[Algebra K B] [Algebra K C] [Algebra B C]}\\
    \texttt{[IsScalarTower K B C] [Module.FaithfullyFlat B C]}\\
    \texttt{[Algebra.WeaklyEtale K C] :}\\
    \texttt{Algebra.WeaklyEtale K B}
    \end{quote}

    The skeleton (mirroring Stacks): from \texttt{Module.FaithfullyFlat
    B C} and the commutative square
    \[
    \begin{tikzcd}
        C \otimes_K C \ar[r] & C \\
        B \otimes_K B \ar[r] \ar[u] & B \ar[u]
    \end{tikzcd}
    \]
    deduce flatness of $C$ as a $B \otimes_K B$-module (the top
    horizontal is flat by hypothesis \texttt{Algebra.WeaklyEtale K C},
    the verticals are flat by \texttt{Module.FaithfullyFlat B C}).
    The standard Mathlib carrier for faithfully flat descent of
    flatness (e.g. \texttt{Module.Flat.of\_faithfullyFlat\_descent}
    or the analogous name) then descends flatness of $B$ as a
    $B \otimes_K B$-module from flatness of $C$. The lemma is
    generic — no field hypothesis on $B$ or $C$ — and is the
    proper Mathlib-target shape; the iter-149+ prover dispatches
    against this new helper file, e.g.\
    \texttt{Proetale/Mathlib/RingTheory/WeaklyEtale/GoDown.lean}.
    The \Cref{lem:isAlgebraic-of-weaklyEtale} consumer at the L195
    call site instantiates the helper with $K, L', L$ and applies
    Steps 2 and 3 above.
